...
class settingControl extends SystemControl{
	 ...
	 * 短信接口设置
	 */
	public function duanxin_settingOp(){
		/**
		 * 读取语言包
		 */
		$lang	= Language::getLangContent();
		...
		if (chksubmit()){
			 ...
			 */
			$obj_validate = new Validate();
			if (trim($_POST['duanxin_isuse']) == '1'){
				$obj_validate->validateparam = array(
					array("input"=>$_POST["duanxin_appid"], "require"=>"true","message"=>Language::get('duanxin_appid_error')),
					array("input"=>$_POST["duanxin_appkey"], "require"=>"true","message"=>Language::get('duanxin_appkey_error'))
				);
			}
			$error = $obj_validate->validate();
			if ($error != ''){
				 ...
				 */
				$update_array = array();
				$update_array['dxuser'] = trim($_POST['dxuser']);//短信接口账号
			    $update_array['dxmima'] = trim($_POST['dxmima']);//短信接口密码
				$update_array['dxqm'] = trim($_POST['dxqm']);//短信特定签名
				$update_array['hyzctz'] = trim($_POST['hyzctz']);
				$update_array['hyzctzjk'] = trim($_POST['hyzctzjk']);
				$update_array['husjqm'] = trim($_POST['husjqm']);
				$update_array['husjqmjk'] = trim($_POST['husjqmjk']);
				
				$update_array['zfbmj'] = trim($_POST['zfbmj']);
				$update_array['zfbmjjk'] = trim($_POST['zfbmjjk']);
				$update_array['zfbsj'] = trim($_POST['zfbsj']);
				$update_array['zfbsjjk'] = trim($_POST['zfbsjjk']);
				$update_array['xxsjdx'] = trim($_POST['xxsjdx']);
				$update_array['xxsjdxjk'] = trim($_POST['xxsjdxjk']);
				$update_array['yfkmj'] = trim($_POST['yfkmj']);
				$update_array['yfkmjjk'] = trim($_POST['yfkmjjk']);
				$update_array['yfksj'] = trim($_POST['yfksj']);
				$update_array['yfksjjk'] = trim($_POST['yfksjjk']);
				$update_array['mjfh'] = trim($_POST['mjfh']);
				$update_array['mjfhjk'] = trim($_POST['mjfhjk']);
		 ...
		 */
		Tpl::output('list_setting',$list_setting);
		Tpl::showpage('setting.duanxin_setting');
	}
	/**
	 * 直通车设置
	 */
