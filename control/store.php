...
//		$store_info['grade_goodslimit']	= $store_info['grade_goodslimit'] == 0 ? $lang['store_none_limit'] : $store_info['goods_count'].'/'.$store_info['grade_goodslimit'];
	...
	public function dorp_imgOp(){
		...
		if (!$file_info) {
            $imgArr = explode('/', $_GET['img_src']);//把路径分成数组--
            if ($imgArr[0] == 'upload' && end($imgArr) !='index.html') {//如果路径的第一个目录是upload，并且最后一个不是index.html就删除。因为图片上传的目录基本上都在upload目录下。同时，upload目录下基本上不会上传其它东西
