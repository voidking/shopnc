<?php
/**
 * 手机找回密码 复制login.php的副本，
 *
 *第15行的文件标头：class login2Control extends BaseHomeControl 注意：loginControl 要改为login2Control
 *
 *
...
class login2Control extends BaseHomeControl {
...
$phone_zc=$_POST['mobile'];//注册填写的手机号  

$user = $GLOBALS['setting_config']['dxuser']; //短信接口账号
$passwd = $GLOBALS['setting_config']['dxmima']; //短信接口密码 
               
$message_mj=$GLOBALS['setting_config']['hyzctz']; //会员注册时的短信内容
$gateway_1 =$GLOBALS['setting_config']['hyzctzjk']; //HTTP_POST接口代码
	...
	public function find_passwordOp(){
		...
		* 忘记密码发送短信----------------------------------------------------------------------------------------------------------
		 */
               ...
               $phone_zc=$_POST['mobile'];//注册填写的手机号  
               $youghu=$_POST['username'];//输入的用户名
               $xinmima=$new_password;//新密码 
               
$user = $GLOBALS['setting_config']['dxuser']; //短信接口账号
$passwd = $GLOBALS['setting_config']['dxmima']; //短信接口密码 
                
$message_mj=$GLOBALS['setting_config']['husjqm']; //手机取回密码短信内容
$gateway_1 =$GLOBALS['setting_config']['husjqmjk']; //HTTP_POST接口 
$gateway_1=iconv("UTF-8","GB2312//IGNORE",$gateway_1);  
$rccc_1 = @file_get_contents($gateway_1);  
