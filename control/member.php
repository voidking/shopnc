...
class memberControl extends BaseMemberControl{
		...
		if(chksubmit()) {
			...
			if ($state_code == 40){
                ...
                if (($order['payment_code'] == 'predeposit')){
					...
					$store_info = $store_model->shopStore(array('member_id'=>$seller['member_id']));
					if($store_info['royalty_rate_fanhe'] > 1) {
						$store_info['royalty_rate_fanhe'] = 0;
					}
					//fanhe
					...
					$log_arr['logtype'] = '0';
					$log_arr['price'] = $order['order_amount']; //fanhe
					...
					$log_arr['logtype'] = '0';
					$log_arr['price'] = -round($order['order_amount']*$store_info['royalty_rate_fanhe'],2);
					$log_arr['desc'] = Language::get('member_change_order_no').$order['order_sn'].'交易完成，支付佣金费用';
					$predeposit_model->savePredepositLog('system',$log_arr);
					//fanhe
					unset($log_arr);
					//买家减少冻结金额
					$log_arr = array();	
			...
			//更改订单状态
			$array['royalty_fanhe'] = round($order['order_amount']*$store_info['royalty_rate_fanhe'],2); //fanhe
    ...
    /* ectap触屏模板修改开始 */
    private function _assignMobileData($page){
