	...
	public function advshowOp(){
		...
		if($ap_class == '2'){
			...
			if(empty($data_select)){
				...
				$content .= "document.write(\"<li>";
				$content .= "<a href=''><img class='scrollLoading' src='' data-url='".$pic."' alt='' width='".$ap_width."' height='".$ap_height."' /></a>";
				$content .= "</li>\");";
			}else{
				...
				foreach ($data as $k=>$v){
				    ...
				    if(file_exists($adv_cache_file)){
						...
						$content .= "<li style ='background:".$clr." url(".$pic.") no-repeat center top;'><a  href='".SiteUrl.DS."index.php?act=advclick&op=advclick&adv_id=".$adv_id."&ap_class=".$ap_class."' target='_blank'></a></li>";
					  }
		            }					
				}
				$content .= "\");";	 
			        ...
			        if(empty($data_select)){
			        	switch ($ap_class){
			        		case '0':
						        ...
						        $content .= "<img style='width:{$width}px;height:{$height}px' border='0' class='scrollLoading' src='' data-url='";
