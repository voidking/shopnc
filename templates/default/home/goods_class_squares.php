...
<?php foreach($output['goods_list'] as $goods){?>
      ...
      <div class="goods-content" nctype_goods="<?php echo $goods['goods_id'];?>" nctype_store="<?php echo $goods['store_id'];?>">
		...
		<?php if(!empty($goods['group_flag'])){?>
          <span  title="<?php echo $lang['goods_class_index_groupbuy'];?>">团购商品</span>
          <?php } else?>
          <?php if(!empty($goods['xianshi_flag'])){ ?>
          <span  title="<?php echo $lang['goods_class_index_xianshi'];?>">限时商品</span>
          <?php } ?></div><?php } ?>
		...
		?>
          ...
          <div class="goods-name"><a href="<?php echo ncUrl(array('act'=>'goods','goods_id'=>$goods['goods_id']),'goods',$goods['store_domain']);?>" target="_blank" title="<?php echo (!empty($goods['goods_subtitle']))?$goods['goods_subtitle']:$goods['goods_name']?>"><?php echo $goods['goods_name'];?><em><?php echo $goods['goods_subtitle'];?></em></a></div>

          <div class="goods-price">
          <!-- S 促销价格显示 -->
<?php if(intval($goods['group_flag']) === 1) { ?>
<!-- 团购 -->
<em class="sale-price" title="<?php echo $lang['goods_class_index_promotion_goods_price'].$lang['nc_colon'].$lang['currency'];?><?php echo $goods['group_price'];?>"><?php echo $lang['currency'];?><?php echo $goods['group_price'];?></em> <em class="market-price" title="<?php echo $lang['goods_class_index_normal_goods_price'].$lang['nc_colon'].$lang['currency'];?><?php echo $goods['goods_store_price'];?>"><?php echo $lang['currency'];?><?php echo $goods['goods_store_price'];?></em>
<?php } else if( intval($goods['xianshi_flag']) === 1) { ?>
<?php $xianshi_price = ncPriceFormat($goods['goods_store_price'] * $goods['xianshi_discount'] / 10);?>
<em class="sale-price" title="<?php echo $lang['goods_class_index_promotion_goods_price'].$lang['nc_colon'].$lang['currency'];?><?php echo $xianshi_price;?>"><?php echo $lang['currency'];?><?php echo $xianshi_price;?></em>
<em class="market-price" title="<?php echo $lang['goods_class_index_normal_goods_price'].$lang['nc_colon'].$lang['currency'];?><?php echo $goods['goods_store_price'];?>"><?php echo $lang['currency'];?><?php echo $goods['goods_store_price'];?></em>
<?php } else { ?>
<em class="sale-price" title="<?php echo $lang['goods_class_index_normal_goods_price'].$lang['nc_colon'].$lang['currency'];?><?php echo $goods['goods_store_price'];?>"><?php echo $lang['currency'];?><?php echo $goods['goods_store_price'];?></em>
<?php } ?>
<!-- E 促销价格显示 --> 
<span class="raty" data-score="<?php echo $goods['store_desccredit'];?>"></span>
  ...
  <?php }else{?>
  <div class="no-results"><?php echo $lang['goods_class_index_no_record'];?></div>
  <?php }?>
</div>