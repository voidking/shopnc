...
<?php include template('store/new_header'); ?>
<link href="<?php echo TEMPLATES_PATH;?>/new-css/shop.css"  rel="stylesheet" type="text/css">
<style type="text/css">body{background:0}#content{width:1200px}.expanded .nc-goods-main.nc-goods-main{width:980px}.expanded .nc-sidebar{width:210px}.nc-goods-main{width:1200px}.wrapper{width:1200px;}</style>
<div class="background clearfix">

  <article id="content">
  <?php include template('store/cur_local'); ?>
    <section class="nc-detail">
        <?php if(!$output['store_self']) { ?>
        <div class="nc-inform"><span><?php echo $lang['goods_index_inform'];?><i></i></span>
          <ul>
      ...
      <div class="nc-detail-bd"> 
        ...
        <aside class="nc-gallery">
          <div class="zoom-section"> 
            <!-- S 默认图片触及显示放大镜效果 -->
            <div class="zoom-small-image"><span class="thumb size360"><a href="<?php echo cthumb($output['goods_image'],'max',$output['goods']['store_id']);?>" class = "nc-zoom" id="zoom1" rel="position: 'inside' , showTitle: false"><img src="<?php echo cthumb($output['goods_image'],'mid',$output['goods']['store_id']);?>" alt="" title=""></a></span></div>
            <!-- S 默认图片触及显示放大镜效果 --> 
            <!-- S 关联默认图片的缩略图 -->
            <nav class="zoom-desc">
              <ul>
                ...
                <!-- S 第一个后其它的有图片时显示 -->
                <li><a href="<?php echo cthumb($val,'max',$output['goods']['store_id']);?>" class="nc-zoom-gallery <?php if($key=='0'){?>hovered<?php }?>" title="" rel="useZoom: 'zoom1', smallImage: '<?php echo cthumb($val,'mid',$output['goods']['store_id']);?>' "> <span class="thumb size60"> <i></i> <img src="<?php echo cthumb($val,'tiny',$output['goods']['store_id']);?>" alt="" onload="javascript:DrawImage(this,60,60);"> </span><b></b> </a></li>
                <?php }?>
                <?php }?>
                <?php }?>
              </ul>
        ...
        <strong><?php echo $output['goods']['goods_subtitle']; ?></strong></div>
          <div class="nc-meta"> 
            <!-- S 商品发布价格 -->
            <dl class="nc-detail-price ">
              <dt><?php echo $lang['goods_index_goods_price'];?><?php echo $lang['nc_colon'];?></dt>
              <dd><strong <?php if($output['group_flag'] || ($output['xianshi_flag']&&$output['start_flag'])) echo "class='del'"; ?>nctype="goods_price"><?php echo $lang['currency'];?><?php echo (($output['goods']['goods_store_price_interval'] == '' || $output['goods']['spec_open'] == '0')? $output['goods']['goods_store_price'] : $output['goods']['goods_store_price_interval']); ?></strong></dd>
            </dl>
            <!-- E 商品发布价格 --> 
            <!-- S 商品促销价格 --> 
            <!-- S 团购 -->
            ...
            <dl clsss="nc-promo">
              ...
              <dd><em class="nc-promo-price-type"><?php echo $lang['nc_groupbuy'];?></em><strong class="nc-promo-price"><?php echo $lang['currency'];?><?php echo $output['group_info']['groupbuy_price'];?></strong></dd>
            </dl>
            <?php } ?>
            <!-- E 团购 --> 
            <!-- S 限时折扣 -->
            ...
            <dl clsss="nc-promo">
              ...
              <dd><em class="nc-promo-price-type"><?php echo $lang['nc_xianshi'];?></em>
                ...
                <!-- 尚未开始 --> 
                <strong nctype="xianshi_price"><?php echo $lang['currency'];?><?php echo ncPriceFormat($output['goods']['goods_store_price']*$output['xianshi_goods']['discount']);?></strong>
                ...
                <?php } else { ?>
                <!-- 已经开始 --> 
                <strong class="nc-promo-price" nctype="xianshi_price"><?php echo $lang['currency'];?><?php echo ncPriceFormat($output['goods']['goods_store_price']*$output['xianshi_goods']['discount']);?></strong>
			...
			<!-- S 累计售出数量 -->
			<li class="tm-ind-item tm-ind-sellCount"><p class="tm-count"><em><a href="#ncGoodsTraded"><?php echo $output['goods']['salenum']; ?></a></em></p><p class="tm-label"><?php echo $lang['goods_index_sold'];?></p></li>
			<!-- E 累计售出数量 -->
			<!-- S 描述相符评分及评价数量 -->
			<li class="tm-ind-item tm-ind-reviewCount tm-line3" id="J_ItemRates"><p class="tm-count" id="comment_num"><em><?php echo $output['goods']['commentnum'];?></em></p><p class="tm-label"><?php echo $lang['goods_index_number_of_consult'];?></p><p class="raty ra" data-score="<?php echo $output['store_info']['store_desccredit'];?>"></p></li>
			<!-- E 描述相符评分及评价数量 --> 
			<!-- S 商品类型及浏览数-->
			<li class="tm-ind-item tm-ind-emPointCount"><a target="_blank"><p class="tm-count"><em><?php if($output['goods']['goods_form'] == '1'){?><?php echo $lang['goods_index_new'];?><?php }else{?><?php echo $lang['goods_index_used'];?><?php }?></em></p><p class="tm-label"><?php echo $lang['goods_index_goods_type'];?></p></a></li></ul>
            <!-- E 商品类型及浏览数-->
          </div>
          ...
          <div class="nc-key"> 
            ...
            <dl>
              ...
              <dd class="nc-figure-input"> <a href="javascript:void(0)" ><i class="icon-minus decrease fl text-hidden"></i></a>
                <input type="text" name="" id="quantity" value="1" size="3" maxlength="6" class="fl" style="border-radius:0;">
                <a href="javascript:void(0)" ><i class="icon-plus increase fl text-hidden"></i></a> <em class="fl ml20">(<?php echo $lang['goods_index_stock'];?><strong nctype="goods_stock"><?php echo $output['goods']['spec_goods_storage']; ?></strong><?php echo $lang['nc_jian'];?>)</em> </dd>
				<div class="clear"></div>
            </dl>
            <!-- E 购买数量及库存 --> 
            <!-- S 提示已选规格及库存不足无法购买 -->
            ...
            <div class="nc-btn clearfix">
              ...
              <a href="javascript:buy('groupbuy');" class="buynow fl text-hidden" title="<?php echo $lang['goods_index_now_buy'];?>"><?php echo $lang['goods_index_now_buy'];?></a><!-- 团购购买--> 
              <!-- S 加入购物车弹出提示框 -->
              <?php } elseif(!empty($output['xianshi_flag']) && !empty($output['start_flag'])) { ?>
              <a href="javascript:buy('buynow');" class="buynow fl text-hidden" title="<?php echo $lang['goods_index_now_buy'];?>"><?php echo $lang['goods_index_now_buy'];?></a><!-- 立即购买-->
              <?php } else { ?>
              <a href="javascript:buy('buynow');" class="buynow fl text-hidden" title="<?php echo $lang['goods_index_now_buy'];?>"><?php echo $lang['goods_index_now_buy'];?></a><!-- 立即购买--> 
              <a href="javascript:buy('');" class="addcart fl ml10 text-hidden" title="<?php echo $lang['goods_index_add_to_cart'];?>"><?php echo $lang['goods_index_add_to_cart'];?><!-- 加入购物车--></a> 
              <!-- S 加入购物车弹出提示框 -->
     ...
     <div class="ncg-store"style=" position: absolute; z-index: 1; top: 60px; right: 0;">
      <?php include template('store/info2');?>
    </div>
        <!--E 店铺信息 --> 
      </div>
      <div class="clear"></div>
        ...
        <dl class="nc-mansong-content">
          ...
          <dd>
              ...
              <span><?php echo $lang['nc_man'];?><em><?php echo ncPriceFormat($rule['price']);?></em><?php echo $lang['nc_yuan'];?>
                ...
                ,<?php echo $lang['nc_reduce'];?><i><?php echo ncPriceFormat($rule['discount']);?></i><?php echo $lang['nc_yuan'].$lang['nc_cash'];?>
                <?php } ?>
                <?php if(!empty($rule['shipping_free'])) { ?>
                ,<i><?php echo $lang['nc_shipping_free'];?></i>
                <?php } ?>
                <?php if(!empty($rule['gift_name'])) { ?>
                ,<?php echo $lang['nc_gift'];?><a href="<?php echo $rule['gift_link'];?>" target="_blank" class="red" ><i><?php echo $rule['gift_name'];?></i></a>
                <?php } ?>
              </span>
              <?php } ?>
          </dd>
    ...
    <!--E 满就送 -->
        ...
        <section class="nc-s-c-s2 ncg-com-list">
          ...
          <div class="content">
            <ul>
              ...
              <li>
				<dl>
					<dt class="goods-name"><a href="<?php echo ncUrl(array('act'=>'goods','goods_id'=>$goods_commend['goods_id']), 'goods');?>" target="_blank" title="<?php echo $goods_commend['goods_name'];?>"><?php echo $goods_commend['goods_name'];?><em></em></a></dt>
					<dd class="goods-pic"><a href="<?php echo ncUrl(array('act'=>'goods','goods_id'=>$goods_commend['goods_id']), 'goods');?>" target="_blank" title=""><img src="<?php echo thumb($goods_commend,'small');?>" alt="<?php echo $goods_commend['goods_name'];?>"></a></dd>
					<dd class="goods-price">商&nbsp;城&nbsp;价：<em class="price"><?php echo $lang['currency'];?><?php echo $goods_commend['goods_store_price'];?></em></dd>
				</dl>
			   </li>
              <?php }?>
              <?php }?>
      ...
      <aside class="nc-sidebar">
        ...
        <div class="content">
          <div class="ncs-goods-code"><p><img src="http://chart.apis.google.com/chart?
cht=qr&&chs=150x150&choe=UTF-8&chld=L|4&chl=<?php echo SiteUrl.'/';?>%2Findex.php
%3Fact%3Dgoods%26goods_id%3D<?php echo $output['goods']['goods_id'];?>" width='150' height='150' alt='<?php echo $output['goods']['goods_name']; ?>'></p> <span class="ncs-goods-code-note"><i></i>扫描二维码，手机查看分享</span> </div>         </div>
      </div>
...
<div id="footer-top">
	...
	<li class="two"><strong>正品行货<em>100%正品保证</em></strong></li>
	<li class="three"><strong>正品行货<em>100%正品保证</em></strong></li>
	<li class="four"><strong>正品行货<em>100%正品保证</em></strong></li>
	<li class="five"><strong>正品行货<em>100%正品保证</em></strong></li>
	</ul>
</div>
</div>
<!--帮助中心-->
    ...
    <ul>
      ...
      <li>
        <dl class="s<?php echo ''.$k+1;?>">
          ...
          <dd><i></i><a href="<?php if($article['article_url'] != '')echo $article['article_url'];else echo ncUrl(array('act'=>'article','article_id'=>$article['article_id']) ,'article');?>" title="<?php echo $article['article_title']; ?>"> <?php echo str_cut($article['article_title'],13);?> </a></dd>
          <?php }?>
          <?php }?>
        </dl>
      </li>
...
<script src="<?php echo RESOURCE_PATH;?>/new-js/jquery.raty/jquery.raty.min.js" type="text/javascript" charset="utf-8"></script> 
