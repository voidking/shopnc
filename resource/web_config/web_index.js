  ...
  var recommend_show = 1;//当前选择的商品推荐
	...
	titles["upload_tit"] = '标题图片(212px × 45px)';
	titles["upload_act"] = '活动图片(212px × 280px)';
	titles["upload_adv"] = '广告图片(212px × 241px)';
...
function add_recommend(){//增加商品推荐
	for (var i = 1; i <= recommend_max; i++){//防止数组下标重复
		if ($(".middle dl[recommend_id='"+i+"']").size()==0) {//编号不存在时添加
			...
			'" type="text" class="w200"></span><span class="right">修改该区域中部推荐商品模块选项卡名称，控制名称字符在4-8字左右，超出范围自动隐藏</span><div class="clear"></div></div></dt><dd><h4 class="dialog-handle-title">推荐商品</h4><ul class="dialog-goodslist-s1 goods-list"><div class="s-tips"><i></i>小提示：单击查询出的商品选中，双击已选择的可以删除，最多8个，保存后生效。</div></ul></dd></dl>');
			$("#recommend_list_form dl dd ul").sortable({ items: 'li' });
			break;
		}
	}
