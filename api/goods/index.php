...
class goods extends APIControl {
	...
	public function html($goods_list,$config){
		...
		foreach($goods_list as $val) {
			...
			$goods_html .= '<li class="c1"><dl>';
			$goods_html .= '<dt class="goods-name"><a href="'.$this->href($val['goods_id'],$val['store_id']).'" title="'.$val['goods_name'].'" target="_blank">'.$val['goods_name'];
			$goods_html .= '</a></dt><dd class="goods-thumb">';
			...
			$goods_html .= '"/></a></dd>';
			$goods_html .= '<dd class="goods-price">'.$this->getLanguage('index_index_store_goods_price').'：<em>'.$this->getLanguage('currency').$val['goods_store_price'];
			$goods_html .= '</em></dd></dl>';
			$goods_html .= '</li>';
		}
		$goods_html .= '</ul>';
