...
class EditionSwitch{
    public function run($edition){
        if($edition == 'pc'){
            $this->runInPcEdition();
        } elseif($edition == 'mobile') {
            $this->runInMobileEdition();
        } else {
            $this->runInPcEdition();
        }
    }
    public function runInPcEdition(){
        $platCookie = $_COOKIE['PLAT'];
        $platGet = $_GET['pt'];
        //var_dump($platCookie, $platGet);exit;
        if(empty($platCookie) && empty($platGet)){
            if(self::isMobile()){
                $this->gotoMobileEdition();
            }
        } elseif(empty($platCookie) && $platGet === 'mobile'){
            $this->setPlatCookies('mobile');
            if(self::isMobile()){
                $this->gotoMobileEdition();
            }
        } elseif(empty($platCookie) && $platGet === 'pc'){
            $this->setPlatCookies('pc');
        } elseif($platCookie === 'mobile' && empty($platGet)){
            if(self::isMobile()){
                $this->gotoMobileEdition();
            }
        } elseif($platCookie === 'mobile' && $platGet === 'mobile'){
            if(self::isMobile()){
                $this->gotoMobileEdition();
            }
        } elseif($platCookie === 'mobile' && $platGet === 'pc'){

            $this->setPlatCookies('pc');
        } elseif($platCookie === 'pc' && empty($platGet)){

        } elseif($platCookie === 'pc' && $platGet === 'mobile'){
            $this->setPlatCookies('mobile');
            if(self::isMobile()){
                $this->gotoMobileEdition();
            }
        } elseif($platCookie === 'pc' && $platGet === 'pc'){

        } else {exit('10');
            $this->delPlatCookies();
        }
    }
    private function setPlatCookies($plat){
        $this->dsetcookie('PLAT', $plat, 3600);
    }
    private function delPlatCookies(){
        $this->dsetcookie('PLAT', '', 0);
    }
    private function dsetcookie($var, $value = '', $life = 0, $prefix = 1, $httponly = false) {
        ...
        $path = $httponly && PHP_VERSION < '5.2.0' ? $cfg['cookiepath'].'; HttpOnly' : $cfg['cookiepath'];

        $secure = $_SERVER['SERVER_PORT'] == 443 ? 1 : 0;
        if(PHP_VERSION < '5.2.0') {
            setcookie($var, $value, $life, $path, $cfg['cookiedomain'], $secure);
        } else {
            setcookie($var, $value, $life, $path, $cfg['cookiedomain'], $secure, $httponly);
        }
    }
    public function runInMobileEdition(){
        if(!self::isMobile()){
            $this->gotoPcEdition();
        }
    }
    ...
    private function gotoPcEdition(){
        global $config;
        $url = $config['site_url'];
        header('Location:' . $url);
        exit();
    }
    private function gotoMobileEdition(){
        global $config;
        if(!empty($config['ectap_domain'])){
            $url = 'http://' . $config['ectap_domain'];
            if($_GET['act'] == 'show_store') {
                $url .= '/index.php?act=show_store&id=' . $_GET['id'];
            }
        } else {
            $url = $config['site_url'];
        }
